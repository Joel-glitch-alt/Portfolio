...
//         DOCKER_IMAGE = 'addition1905/project-six:latest'
//         JAVA_HOME = '/usr/lib/jvm/java-17-openjdk-amd64'
//         SONAR_SCANNER_OPTS = "-Xmx2048m"  // Reduced from 4096m to 2048m
//     }
...
//                             # Build sonar-scanner command with memory optimization
//                             SONAR_CMD="sonar-scanner"
//                             SONAR_CMD="$SONAR_CMD -Dsonar.projectKey=project-six"
//                             SONAR_CMD="$SONAR_CMD -Dsonar.projectName=\\"Project Six\\""
//                             SONAR_CMD="$SONAR_CMD -Dsonar.sources=."
...
//                             SONAR_CMD="$SONAR_CMD -Dsonar.javascript.maxFileSize=1000"
//                             SONAR_CMD="$SONAR_CMD -Dsonar.typescript.maxFileSize=1000"
                    ...
                    withSonarQubeEnv('Sonar-server') {
                        script {
                            try {
                                ...
                                echo "This won't stop the pipeline from continuing..."
                                currentBuild.result = 'UNSTABLE'
                            }
                        }
                    }
        ...
        stage('Quality Gate') {
            steps {
                timeout(time: 5, unit: 'MINUTES') {
                    script {
                        try {
                            // Check if SonarQube analysis completed successfully
                            if (fileExists('.scannerwork/report-task.txt')) {
                                def qg = waitForQualityGate()
                                echo "SonarQube Quality Gate status: ${qg.status}"
                                
                                // Always force Quality Gate to "fail" but continue pipeline
                                if (qg.status == 'OK') {
                                ...
                                echo "⚠️ SonarQube analysis didn't complete - no report-task.txt found"
                                echo "Quality Gate check skipped due to failed SonarQube analysis"
                                currentBuild.result = 'UNSTABLE'
                            }
                        } catch (Exception e) {
                            echo "Quality Gate check failed or timed out: ${e.getMessage()}"
